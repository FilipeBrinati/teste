...
function addJoystick() {
    ...
    joystick.on('move', function (evt, data) {
        ...
        tValue = bValue = lValue = rValue = 0;

        if (forward > 0)
            tValue = Math.abs(forward)
        else if (forward < 0)
            bValue = Math.abs(forward)

        if (turn > 0)
            rValue = Math.abs(turn)
        else if (turn < 0)
            lValue = Math.abs(turn)
    })
...
function render() {
    ...
    playAction = moverJogador(bValue, tValue, lValue, rValue, camera, renderer, placeholder, rotateTo, checkColision, boxBB, placelooker, movimentoLateral, directionalLight, mat4)
